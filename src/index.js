...
async function processEmailsOnce() {
  ...
  try {
    ...
    logger.info(`⏰ Next check in ${CONFIG.EMAIL_CHECK_INTERVAL_MINUTES} minute(s) at ${new Date(Date.now() + CONFIG.EMAIL_CHECK_INTERVAL_MINUTES * 60 * 1000).toLocaleString('id-ID')}`);
...
async function startEmailMonitoring() {
  logger.success(`🔄 Starting continuous email monitoring (every ${CONFIG.EMAIL_CHECK_INTERVAL_MINUTES} minute(s))`);
  
  // Process emails immediately on start
  await processEmailsOnce();
  
  // Set up interval based on configuration
  const intervalMs = CONFIG.EMAIL_CHECK_INTERVAL_MINUTES * 60 * 1000;
  emailCheckInterval = setInterval(async () => {
    await processEmailsOnce();
  }, intervalMs);
  
  logger.success(`📅 Email monitoring scheduled every ${CONFIG.EMAIL_CHECK_INTERVAL_MINUTES} minute(s)`);
}
...
/**
 * Main application entry point - now runs continuously
 */
